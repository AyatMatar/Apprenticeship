...
namespace FinalProject.Repsitories.LeaderRepsitories
{
    ...
    public class LeaderRepsitories : ILeaderRepsitories
    {
        ...
        {
            List<Attachfile> attachfiles = dbcontext.Attachfiles.Where(x => x.assignmentId == assignmentId && !x.isdelete).ToList();

            return attachfiles;
        }
        public Report GetAllReport(int id)
        {
            var assignments = dbcontext.reports.Include(f => f.reportStatus).Where(x => x.reportId == id && !x.isdelete).Include(x=>x.attachfiles).FirstOrDefault();

            return assignments;
        }
        public List< ReportLog> GetAllreportLog(int id)
        {
            var assignments = dbcontext.reportLogs.Include(f => f.report).Include(f=>f.ReportStatus).Where(x => x.reportId == id && !x.isdelete).ToList();
        ...
        public RequestStatus UbdatereportStatus(int reportId, int reportStatusId)
        {
            try
            {
                var assignments = dbcontext.reports.Where(x => x.reportId == reportId && !x.isdelete).FirstOrDefault();
                if(assignments != null)
                {
                    ...
                    dbcontext.SaveChanges();
                    ReportLog reportLog = new ReportLog(reportId, assignments.reportName, assignments.reportDescription, assignments.reportNotes, assignments.reportStatusId, DateTime.Now);
